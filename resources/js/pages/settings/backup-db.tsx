// File: resources/js/Pages/settings/backup-db.tsx
...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Database Backups', href: '/settings/backup' },
];
...
const getStatusBadge = (status: string) => {
  switch (status) {
    case 'completed':
      return <Badge variant="success" className="bg-green-100 text-green-800 border-green-200">Completed</Badge>;
    case 'failed':
      return <Badge variant="destructive" className="bg-red-100 text-red-800 border-red-200">Failed</Badge>;
    case 'running':
      return <Badge variant="outline" className="text-blue-700 border-blue-500 bg-blue-50 animate-pulse">Running</Badge>;
    case 'pending':
      return <Badge variant="secondary" className="text-amber-700 border-amber-400 bg-amber-50">Pending</Badge>;
    default:
...
export default function DbBackup({ records: initialRecords }: { records: Records[] }) {
  ...
  function handleCreateBackup() {
    setIsCreating(true);
    router.put(route('backup.database'), {}, {
      preserveScroll: true,
              ...
              <Table className="min-w-[900px]">
                ...
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    <TableHead className="whitespace-nowrap font-semibold">Date Created</TableHead>
                    ...
                    records.map((record) => (
                      <TableRow key={record.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <TableCell className="font-mono text-sm">
                          <span className={record.file_size && record.file_size !== '-' ? 'text-foreground' : 'text-muted-foreground'}>
